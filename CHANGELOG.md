...
### Changed
- **Tidal coverage** — one backfill wave added 19 `uri_tidal` entries; `ballermann-party-hits` is now complete at 189/189 (#1879).
...
- **Stale playlist copies left behind at a former path are now removed (#1864).** Copying bundled playlists into the runtime directory only ever created or version-bumped files, never removed them. When a shipped playlist moved inside the bundle, the copy at the old path stayed on disk — still discovered, still listed in the picker under the same name, and still playable — so a host could pick a playlist and silently get an outdated copy. On the reporting instance this left 28 files at the legacy flat path, 13 of them duplicates of a current playlist and 8 at an older version; `schlager-klassiker` served 60 songs instead of 96. Startup now deletes runtime copies of shipped playlists that sit anywhere other than where they ship today, provided the current copy is present. User-created playlists, anything under `user/`, and playlists we no longer ship are never touched.
- **The lobby now shows the round duration the game will actually use (#1867).** The chip rendered the duration straight from stored settings while start-game sent `adminState.selectedDuration` — two separate reads, so the lobby could advertise 45s while the server ran a different timer with nothing to see. Both now read the same value. Stored durations are coerced and range-checked on the way in, and an unusable one leaves the current value alone instead of being silently replaced.
- **The lobby shows the round duration the server is really counting (#1867).** `round_duration` was the one create-game setting the server never echoed back: the client posted it once and could not read it again, so every duration on screen was the browser's own intent and a mismatch was invisible by construction — the reported case (lobby "45S", server 30.0s) could only be found in the log. The value is now part of the game state the server broadcasts, and once a game exists the lobby renders that number; if a pending setting differs, both are shown. This matters because the duration is fixed when the game is created and no endpoint changes it afterwards, so a setting edited after the lobby was opened does not apply to the running game — previously it was displayed as though it did. Creating a game also logs the duration it starts with and what the client asked for, so the next report of a wrong timer does not need log archaeology.
- **The admin can no longer boot into an empty page (#1868).** Reloading `/beatify/admin` after completing the wizard could leave both the wizard container and the home view hidden — logo, two header buttons and the version footer, and no way back except clearing `localStorage`. Neither root is visible by default (`#home-view` needs `body.home-mode`, `#wizard-root` needs `body.wizard-active`), so any boot path that ended in neither branch rendered nothing. The boot sequence now ends in a guard that always leaves a usable view up, preferring the home view and falling back to the wizard. A failing `/beatify/api/status` call also routes into the home view instead of writing an error into a container the stylesheet keeps hidden.
- **A round no longer starts when Music Assistant never took the speaker (#1863).** After the playback wait timed out, Beatify accepted the attempt as "MA is probably still buffering" whenever the speaker's `media_title` had changed at all — including when the player reported no active Music Assistant queue, meaning MA had not accepted the request for that speaker in the first place. The round then started with a live timer, no music, and nothing in the log above WARNING. A player that reports an empty `active_queue` is now treated as a failure, so the game pauses and shows the recovery banner instead. Players whose Music Assistant version does not report `active_queue` at all keep the previous buffering tolerance unchanged.
- **The client's round-duration floor matches the server's again (#1867).** `admin/util.js` documented its bounds as a mirror of `const.py` but shipped a minimum of 10 against Python's 15, so a stored duration of 10–14 passed client-side normalisation and was then rejected by the server with a 400. The floor is now 15, and a test parses `const.py` so the two cannot drift apart unnoticed. No duration the wizard offers (30 / 45 / 60) is affected.
- **A round no longer depends on its own timer task, or on an open browser, to end (#1865).** The round timer is a single `asyncio` task that sleeps to the deadline and then ends the round; if it is cancelled, raises, or blocks in the time-up announcement, nothing on the server noticed. The only rescue was the countdown running in a player's browser, so a TV-only setup or a host with a locked phone sat on `PLAYING` indefinitely. A periodic check owned by the config entry now ends any round whose deadline has passed by more than a short grace period, independent of the round's own task and of any connected client. It stays a backstop: a timer running slightly late still ends its own round, and `end_round` is idempotent so the two cannot double-fire.

### Changed
- **Tidal coverage** — two backfill waves added 38 `uri_tidal` entries across two playlists (`ballermann-party-hits`, `100-greatest-rock-songs`) (#1873, #1876).

### Removed
- **Dead flat-admin timer selector.** `setupTimerSelector` / `updateTimerButtons` / `setTimerDuration` bound to `.timer-btn`, which no longer exists in any template — the wizard uses `.chip[data-duration]`. `setupTimerSelector` had no caller, so none of it could run (#1867).
